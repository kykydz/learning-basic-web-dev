// buggy-server.js - Server utama yang sudah diperbaiki (versi 3)
...
// Handler ini sudah diperbaiki agar tidak infinite loop.
app.post("/count", dhi237yh72384y38iririri);

// --- Perbaikan untuk endpoint /async-error ---
...
// agar server tidak crash dan dapat mengirim respons error yang sesuai.
app.post("/async-error", async (req, res) => {
  try {
    // Simulasi error asinkron.
    // Promise.reject akan langsung melompat ke blok catch.
    await Promise.reject("Oops! An async error occurred on purpose.");
    // Baris ini tidak akan tercapai karena promise ditolak.
    res.send("This should not be reached.");
  } catch (error) {
    console.error("Async error caught:", error);
    // Mengirim respons error ke klien dengan status 500 (Internal Server Error).
